...
const Yo = `@import"https://fonts.googleapis.com/css?family=Roboto&display=swap";h1{font-family:Roboto,sans-serif}a{color:#f0f8ff}.container{background-image:var(--33b4b60e);background-repeat:no-repeat;background-size:cover;font-family:Roboto,sans-serif;display:grid;aspect-ratio:3 / 2;max-width:1200px;grid-template-rows:10% auto 10%;grid-template-columns:1fr 1fr;background-color:#000;grid-auto-rows:150px;color:#fff}.content{grid-column-start:1;grid-column-end:3;align-self:center;justify-self:center}.column{padding:30px}h1{margin:0;font-size:10vw}.btn{padding:10px 20px;border-radius:20px;border:none;background-color:#fff;color:#000}
`, Zo = (e, t) => {
